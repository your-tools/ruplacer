...
impl std::fmt::Display for Stats {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        write!(f, "{} {} on {} matching {}", self.num_replacements, replacements_string, self.matching_files, file_string)
    }
}
...
mod tests {
    ...
    fn test_stats_to_string() {
        let stats = Stats{ matching_files:2, num_replacements:4 };
        let actual = stats.to_string();
        assert_eq!(actual, "4 replacements on 2 matching files");

        let stats = Stats{ matching_files:1, num_replacements:2 };
        let actual = stats.to_string();
